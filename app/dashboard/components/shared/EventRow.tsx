...
const SOURCE_LABEL: Record<string, string> = {
  ...
  ics_import: "Imported from ICS file",
...
export function EventRow({
  ...
  const [draft, setDraft] = useState({
    ...
    reminder_offset_minutes: (event.reminder_offset_minutes ?? DEFAULT_REMINDER_OFFSET_MINUTES) as ReminderOffsetMinutes,
  })
  const [saving, setSaving] = useState(false)
  const isIcs = event.source === "ics_import"
  const memberColor = event.member_name && kids
  ...
  async function save() {
    ...
    await onUpdate(event.id, {
      ...
      member_name: draft.member_name || null,
  ...
  return (
    <>
            ...
            {!editing ? (
              <>
                <div style={{ display: "flex", flexDirection: "column", gap: "0.75rem", marginBottom: "1.5rem" }}>
                  ...
                  {event.member_name && (
                    <div><div style={fieldLabelStyle}>Family Member</div><span style={{ display: "inline-block", marginTop: "0.2rem", fontSize: "0.78rem", color: memberColor, background: `${memberColor}20`, borderRadius: "10px", padding: "0.2rem 0.6rem", fontWeight: 700 }}>{event.member_name}</span></div>
                  )}
              ...
              <>
                <div style={{ display: "flex", flexDirection: "column", gap: "0.75rem", marginBottom: "1.25rem" }}>
                  <div><label style={fieldLabelStyle}>Title</label><input autoFocus value={draft.title} onChange={(e) => setDraft((current) => ({ ...current, title: e.target.value }))} style={{ ...inputSt, marginTop: "0.25rem" }} placeholder="Event title" /></div>
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0.625rem" }}>
                    <div><label style={fieldLabelStyle}>Date</label><input type="date" value={draft.event_date} onChange={(e) => setDraft((current) => ({ ...current, event_date: e.target.value }))} style={{ ...inputSt, marginTop: "0.25rem", colorScheme: "dark" }} /></div>
                    <div><label style={fieldLabelStyle}>Start Time</label><input type="time" value={draft.start_time} onChange={(e) => setDraft((current) => ({ ...current, start_time: e.target.value }))} style={{ ...inputSt, marginTop: "0.25rem", colorScheme: "dark" }} /></div>
                  </div>
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0.625rem" }}>
                    <div><label style={fieldLabelStyle}>End Time</label><input type="time" value={draft.end_time} onChange={(e) => setDraft((current) => ({ ...current, end_time: e.target.value }))} style={{ ...inputSt, marginTop: "0.25rem", colorScheme: "dark" }} /></div>
                    {kids && kids.length > 0 && (
                      <div><label style={fieldLabelStyle}>Member</label><select value={draft.member_name} onChange={(e) => setDraft((current) => ({ ...current, member_name: e.target.value }))} style={{ ...inputSt, marginTop: "0.25rem", cursor: "pointer" }}><option value="">Family</option>{kids.map((kid) => <option key={`${kid.kind}-${kid.name}`} value={kid.name}>{kid.name}</option>)}</select></div>
                    )}
                  </div>
                  <div>
                    <label style={fieldLabelStyle}>Reminder</label>
                    <select value={String(draft.reminder_offset_minutes)} onChange={(e) => setDraft((current) => ({ ...current, reminder_offset_minutes: Number(e.target.value) as ReminderOffsetMinutes }))} style={{ ...inputSt, marginTop: "0.25rem", cursor: "pointer" }}>
                      {REMINDER_OFFSET_OPTIONS.map((option) => (
                        <option key={option.value} value={option.value}>
                          {getReminderOffsetLabel(option.value, "event", !!draft.start_time)}
                        </option>
                      ))}
                    </select>
                  </div>
      ...
      <div onClick={() => { setShowDetail(true); setEditing(false) }} style={{ display: "flex", alignItems: "center", gap: "0.75rem", padding: "0.625rem 0.875rem", background: isIcs ? "rgba(129,140,248,0.04)" : "rgba(255,255,255,0.03)", borderRadius: "10px", border: `1px solid ${isIcs ? "rgba(129,140,248,0.2)" : "var(--border)"}`, cursor: "pointer" }}>
        ...
        {event.member_name && <span style={{ fontSize: "0.62rem", color: memberColor, background: `${memberColor}20`, borderRadius: "10px", padding: "0.1rem 0.45rem", fontWeight: 700, flexShrink: 0 }}>{event.member_name.split(" ")[0]}</span>}
