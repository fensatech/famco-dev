...
export function SidebarNav({ tab, onTab, scannedCount, pendingTaskCount }: Props) {
  return (
    <aside style={{
      ...
      <button onClick={() => onTab("home")} style={{ display: "flex", alignItems: "center", gap: "0.625rem", marginBottom: "2rem", padding: "0 0.5rem", background: "none", border: "none", cursor: "pointer", width: "100%", textAlign: "left" }}>
          ...
          return (
            <button key={id} onClick={() => onTab(id)} style={{
              ...
              {id === "insights" && scannedCount > 0 && (
                <span style={{ marginLeft: "auto", background: "#fbbf24", color: "#000", borderRadius: "10px", fontSize: "0.65rem", fontWeight: 700, padding: "0.1rem 0.45rem" }}>{scannedCount > 99 ? "99+" : scannedCount}</span>
              )}
              {id === "tasks" && pendingTaskCount > 0 && (
                <span style={{ marginLeft: "auto", background: "#f472b6", color: "#fff", borderRadius: "10px", fontSize: "0.65rem", fontWeight: 700, padding: "0.1rem 0.45rem" }}>{pendingTaskCount}</span>
              )}
            </button>
          )
        })}
